...
SUBMODULE (formul_3p1_id) formul_3p1_methods
 ...
 ! END PROCEDURE construct_formul_3p1_bns_spacings
  ...
  MODULE PROCEDURE analyze_constraint
    ...
    CALL THIS% abs_values_in( 0.0D0, 1.0D07, constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
                              unit_analysis, cnt_m7 )

    CALL THIS% abs_values_in( 1.0D-7, 1.0D-6, constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
                              unit_analysis, cnt_m6 )

    CALL THIS% abs_values_in( 1.0D-6, 1.0D-5, constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
                              unit_analysis, cnt_m5 )

    CALL THIS% abs_values_in( 1.0D-5, 1.0D-4, constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
                              unit_analysis, cnt_m4 )

    CALL THIS% abs_values_in( 1.0D-4, 1.0D-3, constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
                              unit_analysis, cnt_m3 )

    CALL THIS% abs_values_in( 1.0D-3, 1.0D-2, constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
                              unit_analysis, cnt_m2 )

    CALL THIS% abs_values_in( 1.0D-2, 1.0D-1, constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
                              unit_analysis, cnt_m1 )

    CALL THIS% abs_values_in( 1.0D-1, 1.0D0, constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
                              unit_analysis, cnt_0 )

    CALL THIS% abs_values_in( 1.0D0, 1D+1, constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
                              unit_analysis, cnt_p1 )

    CALL THIS% abs_values_in( 1.0D+1, 1.0D+2, constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
                              unit_analysis, cnt_p2 )

    CALL THIS% abs_values_in( 1.0D+2, 1.0D+3, constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
                              unit_analysis, cnt_p3 )

    CALL THIS% abs_values_in( 1.0D+3, HUGE(DBLE(1)), constraint, nx, ny, nz, &
                              THIS% export_constraints_details, &
